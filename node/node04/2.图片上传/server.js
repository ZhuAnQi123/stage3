...
var multer  = require('multer')// 创建multer对象 
var upload = multer({})
const app = express()

app.use('/public',express.static(path.join(__dirname,'./www')))//创建静态资源路径

// 上传图片必须用post方式
app.post('/file',upload.single('xixi'),(req,res)=>{   //postman也要加上xixi
  ...
  // 👇让后缀名 和源文件保持一致，将后缀名取出放在ext
  let ext = originalname.split('.')[1]
  fs.writeFile(path.join(__dirname, `./www/${name}.${ext}`),buffer,(err)=>{ //👈将buffer写入到文件内部
     if(err){
