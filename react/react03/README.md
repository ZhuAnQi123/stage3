...
控制地址栏的改变---to 
    Link to='' exact
    NavLink exact to='' activeClassName='' className=''

Route 控制组件的渲染 path exact component||render||children  
Switch 百里挑一 只返回第一个匹配到的组件--用于做404
Redirect 重定向
```
<HashRouter>
...
```
#### 编程式导航和声明式导航（寻找路由对象）
1.<Link><NavLink>实现路由的跳转 声明式导航体
2.通过js路由对象的方式叫做编程式导航 push replace go  goBack goForwd 
注意 :正常创建的组件是没有路由对象的
...
⚪query[传递的是字符串]   /hehe?us=123&ps=123
